...
export default function InternetBankingChannelPage() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8000/all-rapports")
      .then(res => {
        // The API returns { rapport_analytique, details: [ ... ] }
        const allRows = res.data.donnees || []
        // Filter for 'Internet Banking' channel
        const filtered = allRows.filter((row: ReclamationRow) => row.canal === "Internet Banking")
        setRows(filtered)
        setLoading(false)
      })
      .catch(() => {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold mb-4">Réclamations - Internet Banking</h1>
