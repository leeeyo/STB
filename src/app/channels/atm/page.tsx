...
export default function ATMChannelPage() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8000/all-rapports")
      .then(res => {
        ...
        const filtered = allRows.filter((row: ReclamationRow) => row.canal && row.canal.trim().toLowerCase() === "gab/atm")
        setRows(filtered)
        setLoading(false)
      })
      .catch(() => {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold mb-4">Réclamations - GAB/ATM</h1>
